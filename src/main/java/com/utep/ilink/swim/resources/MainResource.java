...
@Path("/compose")
@Api(value = "Main entry point", description = "Construction of workflows")
@Produces({ "application/json" })
public class MainResource {

  private final String APPNAME = "[workflow-composition]:\t";
  private final String ERROR = "[ERROR]:\t";
  private final String TRANSFORMATIONCOMPONENT_PREFIX_DEFAULT = "Transformation_Component";
  ...
  public String compose(
    ...
    while (executableModels != null && !executableModels.isEmpty() && !target.isEmpty()) {
      depth += 2;
      System.out.println(APPNAME + "Starting depth " + depth);
      while (!executableModels.isEmpty() && !target.isEmpty()) {

        System.out.println(APPNAME + " calling heuristic");
        Model model = getMaxValue(new ArrayList<>(executableModels), new HashSet<>(target),
            new HashSet<>(collectedData), new HashSet<>(visitedModels), modelCatalog);
        
        System.out.println(APPNAME + " heuristic returned model id: " + model.getId());
  ...
  }// end endpoint compose

  // Heuristic for retrieving a component from a set of components
  // The heuristic is based on number of target variables generated
  /**
   * Helper function: calls a recursive call with the given threshold to look into
   * the future
   * 
   * @param components
   ...
   * @return
   */
  public Model getMaxValue(ArrayList<Model> components, HashSet<Data> targetVariables, HashSet<Data> availableVariables,
      HashSet<String> visitedComponents,
      ModelCatalog modelCatalog) {

    // a call was made with invalid parameters, return null
    if (components == null || components.isEmpty() || targetVariables == null || targetVariables.isEmpty()
        || availableVariables == null || availableVariables.isEmpty())
      return null;

    int stepsToAnalyze = 2;
    // HashSet<Variable> availableVariablesSet = new HashSet<>();
    // for (Variable variable : availableVariablesSet) {
    // availableVariablesSet.add(variable);
    // }

    return getMaxTargetVariables(stepsToAnalyze, components, targetVariables, availableVariables,
        visitedComponents,
        modelCatalog);
  }

  /**
   * Recursive method: heuristic function for finding model to analyze next
   * 
   * @param threshold
   * @param components
   * @param targetVariablesSet
   * @param availableVariablesSet
   * @return
   */
  public Model getMaxTargetVariables(int stepsToAnalyze, ArrayList<Model> components, HashSet<Data> targetVariables,
      HashSet<Data> availableVariables, HashSet<String> visitedComponents,
      ModelCatalog modelCatalog) {

    // a call was made with invalid parameters, return null
    if (components == null || components.isEmpty() || targetVariables == null || targetVariables.isEmpty()
        || availableVariables == null || availableVariables.size() == 0)
      return null;

    if (stepsToAnalyze == 0) {
      System.out.println("No component analyzed produces target variables");
      return components.get(0); // return first component
    }

    HashSet<String> targetVariablesSet = new HashSet<>();

    // get ids of variables only
    for (Data targetVariable : targetVariables) {
      targetVariablesSet.add(targetVariable.getId());
    }

    int maxVariablesProduced = 0;
    int variablesProduced = 0;
    Model component = null;

    for (Model componentToAnalyze : components) {
      variablesProduced = 0;
      for (Data output : componentToAnalyze.getOutputs()) {
        if (targetVariablesSet.contains(output.getId())) {
          variablesProduced++;
        }
      }
      if (variablesProduced > maxVariablesProduced) {
        maxVariablesProduced = variablesProduced;
        component = componentToAnalyze;
      }
    }
    if (component != null && maxVariablesProduced != 0) {
      return component;
    }

    // Creating a map of variables and components, (provenance)
    // null in key (model) means it is provided by the request
    HashMap<String, Model> variableModelMap = new HashMap<>();

    for (Model componentToAnalyze : components) {
      for (Data output : componentToAnalyze.getOutputs()) {
        availableVariables.add(output);
        variableModelMap.putIfAbsent(output.getId(), componentToAnalyze);
      }
      visitedComponents.add(componentToAnalyze.getId());
    }

    HashMap<Model, Model> executableComponentsNextStep = getComponents(availableVariables, variableModelMap,
        visitedComponents,
        modelCatalog);

    Model processCollected = getMaxTargetVariables(stepsToAnalyze - 1,
        new ArrayList<Model>(executableComponentsNextStep.keySet()), targetVariables, availableVariables,
        visitedComponents, modelCatalog);

    if (processCollected == null)
      return components.get(0); // returns first element of current components as there is not next step
    return executableComponentsNextStep.get(processCollected);

  }

  /**
   * Returns a key-value pair, the key is the process of the next step to be
   * executed, the value is the process that generated data for that process to be
   * executed
   * 
   * @param availableVariablesSet
   * @param variableModelMap
   * @return
   */
  private HashMap<Model, Model> getComponents(HashSet<Data> availableVariables,
      HashMap<String, Model> variableModelMap,
      HashSet<String> visitedModels,
      ModelCatalog modelCatalog) {

    ArrayList<Model> models = getExecutableModels(
        availableVariables,
        visitedModels,
        modelCatalog);

    HashMap<Model, Model> executableComponentsNextStep = new HashMap<>();

    for (Model model : models) {
      for (Data input : model.getInputs()) {
        // avoid referencing a variable that came from the request
        if (variableModelMap.containsKey(input.getId()) && variableModelMap.get(input.getId()) != null) {
          executableComponentsNextStep.put(model, variableModelMap.get(input.getId()));
          break;
        }
      }
    }

    System.out.println(APPNAME + "Matched: " + executableComponentsNextStep.size() + " out of " +  models.size());

    if (models.size() != executableComponentsNextStep.size()) {
      System.out.println(APPNAME + ERROR + "An error occurred while pairing components and pre-components.");
    }

    return executableComponentsNextStep;
